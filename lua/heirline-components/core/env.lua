...
--- The `M.modes` entry for the current mode.
--- `nvim_get_mode().mode` is the full mode string (e.g. "niI", "Rv"), which is
--- what `M.modes` is keyed on. Anything the table does not list falls back to
--- the bare mode letter, and finally to normal.
---@return string[] # The `{ text, hl }` pair of the current mode.
function M.mode_entry()
  local mode = vim.api.nvim_get_mode().mode
  return M.modes[mode] or M.modes[mode:sub(1, 1)] or M.modes["n"]
end
...
M.buf_matchers = {
  ...
  bufname = function(pattern_list, bufnr)
    return pattern_match(vim.fs.basename(vim.api.nvim_buf_get_name(bufnr or 0)), pattern_list)
  end,
}
