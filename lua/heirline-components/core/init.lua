...
--- Resolve a window *number* — the value encoded in a statusline click region —
--- to the window id to act on.
---@param winnr integer
---@return integer # Window id, or 0 (the current window) when there is no such window.
local function winid_from_number(winnr)
  ...
  return function(self)
    ...
    if opts.max_depth and opts.max_depth > 0 then
      ...
      if start_idx > 0 then
        table.insert(children, {
          provider = utils.get_icon("Ellipsis") .. opts.separator,
        })
      end
    end
    ...
    for i, d in ipairs(data) do
      if i > start_idx then
        local child = {
          ...
          on_click = { -- add on click function
            ...
            callback = function(_, minwid)
              ...
              vim.api.nvim_win_set_cursor(winid_from_number(winnr), { lnum, col })
            end,
        ...
        if opts.icon.enabled then -- add icon and highlight if enabled.
          ...
          local hl_exists = next(vim.api.nvim_get_hl(0, { name = hlgroup, create = false })) ~= nil
          table.insert(child, 1, {
            provider = string.format("%s ", d.icon),
            hl = (hl and hl_exists) and hlgroup or nil,
          })
        end
    ...
    if opts.padding.left > 0 then -- add left padding.
      table.insert(children, 1, {
        provider = core_utils.pad_string(" ", { left = opts.padding.left - 1 }),
      })
    end
    if opts.padding.right > 0 then -- add right padding.
      table.insert(children, {
        provider = core_utils.pad_string(" ", { right = opts.padding.right - 1 }),
      })
    end
...
function M.separated_path(opts)
  ...
  return function(self)
    ...
    local data = vim.split(path, opts.delimiter, { trimempty = true })
    local children = {}
    -- add prefix if needed, use the separator if true, or use the provided character.
    if opts.prefix and not vim.tbl_isempty(data) then
      table.insert(children, { provider = opts.prefix == true and opts.separator or opts.prefix })
  ...
  vim.api.nvim_create_autocmd({ "UIEnter" }, {
    ...
    callback = function()
      ...
      for _, buf in ipairs(current_tab_bufs) do
        if not buf_utils.is_valid(buf) then
          goto continue
        end
        bufs = vim.t.bufs
...
---@return table lsp # A table like  { progress = {} }
function M.lsp_progress()
  local lsp = { progress = {} }

  -- `LspProgress` is emitted by the default `$/progress` handler for every
  -- work-done notification, with `params.value` already carrying the title over
  -- from `begin` to `report`/`end`. Patch nothing, listen instead.
  vim.api.nvim_create_autocmd("LspProgress", {
    desc = "Track LSP progress for the heirline-components statusline",
    callback = function(ev)
      local params, value = ev.data.params, ev.data.params.value
      if type(value) ~= "table" then
        return
      end
      local progress, id = lsp.progress, ("%s.%s"):format(ev.data.client_id, params.token)
      progress[id] = progress[id] and utils.extend_tbl(progress[id], value) or value
