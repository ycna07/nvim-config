...
function M.tabline_conditional_padding(opts)
  return extend_tbl({
    ...
    condition = function(self)
      ...
      local pattern = (opts and opts.pattern)
        or {
          filetype = { "aerial", "dapui_.", "dap%-repl", "neo%-tree", "NvimTree", "edgy", "calltree" },
          buftype = { "nofile" },
        }
      return condition.buffer_matches(pattern, vim.api.nvim_win_get_buf(self.winid))
    end,
...
function M.nav(opts)
  ...
  return M.builder(core_utils.setup_providers(opts, { "ruler", "percentage", "scrollbar" }))
end
...
function M.cmd_info(opts)
  ...
  return M.builder(core_utils.setup_providers(opts, { "macro_recording", "search_count", "showcmd" }))
end
...
function M.mode(opts)
  ...
  return M.builder(core_utils.setup_providers(opts, { "mode_text", "str", "paste", "spell" }))
end
...
function M.git_diff(opts)
  ...
  return M.builder(core_utils.setup_providers(opts, { "added", "changed", "removed" }, function(p_opts, p)
...
function M.diagnostics(opts)
  ...
  return M.builder(core_utils.setup_providers(opts, { "ERROR", "WARN", "INFO", "HINT" }, function(p_opts, p)
    local out = core_utils.build_provider(p_opts, p)
    if out then
...
function M.lsp(opts)
  ...
  return M.builder(core_utils.setup_providers(opts, { "lsp_progress", "lsp_client_names" }, function(p_opts, p, i)
    return p_opts
        and {
          flexible = i,
  ...
  opts = extend_tbl({
    compiler_build_type = {
      condition = function()
        ...
        if is_available("compiler.nvim") and filetype == "c" or filetype == "java" then
          return true
        else
          return false
        end
...
function M.neotree(opts)
  opts = extend_tbl({
    neotree = {
      condition = function()
        return is_available("neo-tree.nvim")
      end,
      padding = { left = 2, right = 2 },
    },
    hl = hl.get_attributes("winbar"),
...
function M.aerial(opts)
  opts = extend_tbl({
    aerial = {
      condition = function()
        return is_available("aerial.nvim")
      end,
      padding = { left = 2, right = 2 },
    },
    hl = hl.get_attributes("winbar"),
...
function M.zen_mode(opts)
  opts = extend_tbl({
    zen_mode = {
      condition = function()
        return is_available("zen-mode.nvim")
      end,
      padding = { left = 2, right = 2 },
    },
    hl = hl.get_attributes("winbar"),
