...
function M.showcmd(opts)
  ...
  return core_utils.stylize(("%%%d.%d(%%S%%)"):format(opts.minwid, opts.maxwid), opts)
end
...
function M.file_encoding(opts)
  return function(self)
    ...
    return core_utils.stylize(string.upper(buf_enc ~= "" and buf_enc or vim.o.enc), opts)
  end
end
...
function M.file_format(opts)
  return function(self)
    ...
    return core_utils.stylize(string.upper(buf_format ~= "" and buf_format or vim.o.fileformat), opts)
  end
end
...
function M.file_modified(opts)
  ...
  return function(self)
    return core_utils.stylize(condition.file_modified((self or {}).bufnr) and opts.str or nil, opts)
  end
end
...
function M.file_read_only(opts)
  ...
  return function(self)
    return core_utils.stylize(condition.file_read_only((self or {}).bufnr) and opts.str or nil, opts)
  end
end
...
function M.file_icon(opts)
  return function(self)
    return core_utils.stylize(core_utils.icon_provider(self and self.bufnr or 0), opts)
  end
end
...
function M.git_branch(opts)
  return function(self)
    return core_utils.stylize(vim.b[self and self.bufnr or 0].gitsigns_head or "", opts)
  end
end
...
function M.lsp_progress(opts)
  ...
  return function()
    ...
    return core_utils.stylize(Lsp and (spinner[math.floor(vim.uv.hrtime() / 12e7) % #spinner + 1] .. table.concat({
      Lsp.title or "",
      Lsp.message or "",
    ...
    if opts.integrations.conform and package.loaded["conform"] then
      vim.list_extend(
        ...
        vim.tbl_map(function(c)
          return c.name
        end, require("conform").list_formatters_to_run(bufnr))
      )
    end
...
function M.virtual_env(opts)
  ...
  return function()
    ...
    elseif opts.conda.enabled and conda then
      if conda ~= "base" or not opts.conda.ignore_base then
        env_str = conda
      end
    end
    if env_str then
      return core_utils.stylize(opts.format and opts.format:format(env_str) or env_str, opts)
    end
  end
end
...
function M.treesitter_status(opts)
  return function(self)
    local ft = vim.bo[(self and self.bufnr) or 0].filetype
    local lang = vim.treesitter.language.get_lang(ft)
    local ok = lang ~= nil and vim.treesitter.language.add(lang) == true
    return core_utils.stylize(ok and "TS" or "", opts)
  end
end
...
function M.compiler_state(opts)
  ...
  return function()
    if is_available("compiler.nvim") or is_available("overseer.nvim") and not ovs then
