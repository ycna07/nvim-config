...
function M.nav_to(tabnr)
  if tabnr > #vim.t.bufs or tabnr < 1 then
    vim.notify(("No tab #%d"):format(tabnr), vim.log.levels.WARN)
  else
...
function M.wipe(bufnr, force)
  ...
  -- window is closed.
  if is_last_valid_window() then
    close_all_nofile_bufs()
  end
